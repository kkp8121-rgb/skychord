...
function createServer() {
  return http.createServer((req, res) => {
    let relative;
    try { relative = decodeURIComponent(new URL(req.url, 'http://localhost').pathname).replace(/^\/skychord(?=\/|$)/, '').replace(/^\/+/, ''); }
...
if (require.main === module) createServer().listen(Number(process.env.PORT) || 4174, '127.0.0.1', () => console.log('SKYCHORD local server ready.'));
